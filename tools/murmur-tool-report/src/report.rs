...
/// Re-exported so the document's timestamps and their formatter stay one import apart.
/// The rendering lives in `libs/murmur-time`, shared with `murmur-tool-corpus`.
