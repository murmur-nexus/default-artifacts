...
//! count. `total` and `failed` are computed by the caller.
//!
...
//!
//! This is the single, shared home of `detect_format`/`parse_cargo`/
//! `parse_pytest`/`parse_go`/`parse_jest`. It was extracted verbatim from
//! `murmur-tool-test-report`'s `src/parse.rs` so that both that native tool and
//! the WASM `murmur-hook-regression-verifier` (which cannot invoke the tool
//! out-of-process) reuse one copy of the parsers. The crate has no native-only
//! dependencies, so it builds for `wasm32-wasip2` as well as native targets.
...
// ── unit tests ──────────────────────────────────────────────────────────────
// These exercise the parsers directly as pure functions. The heavier,
// real-filesystem behaviour of the native tool that wraps them remains covered
// by `tools/murmur-tool-test-report/tests/test_report_integration.rs`.
#[cfg(test)]
mod tests {
    use super::*;
    ...
    fn detect_recognizes_each_format() {
        assert_eq!(
            detect_format("running 1 test\ntest result: ok. 1 passed; 0 failed").as_deref(),
            Some("cargo_test")
        );
        assert_eq!(
            detect_format("=== test session starts ===\n=== 1 passed in 0.01s ===").as_deref(),
            Some("pytest")
        );
        assert_eq!(
            detect_format("--- FAIL: TestFoo (0.00s)").as_deref(),
            Some("go_test")
        );
        assert_eq!(
            detect_format("FAIL src/a.test.js\nTests: 1 failed, 2 passed").as_deref(),
            Some("jest")
        );
        assert_eq!(detect_format("nothing to see here"), None);
    }

    #[test]
    fn cargo_parses_passed_count_and_failure_name() {
        let raw = "\
running 2 tests
test alpha ... ok
test beta ... FAILED
...
---- beta stdout ----
thread 'beta' panicked at src/lib.rs:42:5:
...
test result: FAILED. 1 passed; 1 failed; 0 ignored";
        let (failures, passed) = parse_cargo(raw);
        assert_eq!(passed, 1);
        assert_eq!(failures.len(), 1);
        assert_eq!(failures[0].test_name, "beta");
        assert_eq!(failures[0].file.as_deref(), Some("src/lib.rs"));
        assert_eq!(failures[0].line, Some(42));
    }

    #[test]
    fn pytest_parses_passed_count_and_failure() {
        let raw = "\
=== test session starts ===
=========================== FAILURES ===========================
_________________________ test_beta ___________________________
    def test_beta():
>       assert 1 == 2
E       assert 1 == 2
...
=========================== 1 failed, 3 passed in 0.05s ===========================";
        let (failures, passed) = parse_pytest(raw);
        assert_eq!(passed, 3);
        assert_eq!(failures.len(), 1);
        assert_eq!(failures[0].exception, "AssertionError");
        assert_eq!(failures[0].test_name, "tests/test_x.py::test_beta");
    }

    #[test]
    fn go_counts_pass_and_fail() {
        let raw = "\
=== RUN   TestAlpha
--- PASS: TestAlpha (0.00s)
=== RUN   TestBeta
--- FAIL: TestBeta (0.00s)
    beta_test.go:12: expected 1 got 2
FAIL";
        let (failures, passed) = parse_go(raw);
        assert_eq!(passed, 1);
        assert_eq!(failures.len(), 1);
        assert_eq!(failures[0].test_name, "TestBeta");
        assert_eq!(failures[0].line, Some(12));
    }

    #[test]
    fn jest_parses_tally_and_failure() {
        let raw = "\
FAIL src/a.test.js
  \u{25cf} suite \u{203a} does a thing

    expect(received).toBe(expected)

      at Object.<anonymous> (src/a.test.js:5:20)

Tests:       1 failed, 2 passed, 3 total";
        let (failures, passed) = parse_jest(raw);
        assert_eq!(passed, 2);
        assert_eq!(failures.len(), 1);
        assert_eq!(failures[0].test_name, "suite \u{203a} does a thing");
        assert_eq!(failures[0].exception, "AssertionError");
    }
}
